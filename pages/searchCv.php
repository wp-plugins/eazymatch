...
class EmolCvSearchPage
{
	...
	*/
	 ...
	 * Class constructor
	 */
	function EmolCvSearchPage($slug,$function='')
	{
	...
	function getContent()
	{
        ...
        if( $items_totaal == 0 ){
            $searchHtml = '<div class="emol-no-results">'.get_option( 'emol_cv_no_result' ).'</div>';
        } else {
		    //max amount per page
		    ...
		    foreach($cvs as $cv){
			    ...
			    $searchHtml .= $img;
			    $searchHtml .= '<div class="eazymatch_cv_title"><a href="'.$cv_url.'">('.$cv['id'].') '.$cv['title'].'</a>';
			    $searchHtml .= '<div class="eazymatch_cv_city"><a href="'.$cv_url.'">'.strtoupper($cv['Person']['Preferedaddress']['city']).'</a></div></div>';
			    $searchHtml .= '<div class="eazymatch_cv_body">'.strip_tags(nl2br($cv['description'])).'</div>';
			    $searchHtml .= '<div class="eazymatch_cv_toolbar"><a href="'.$cv_url.'">'.EMOL_SEARCH_READMORE.'</a> | <a href="'.$react_url.'">'.EMOL_CVSEARCH_APPLY.'</a> </div>';
